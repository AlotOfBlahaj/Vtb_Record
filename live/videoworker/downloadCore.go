...
func downloadByStreamlink(video *interfaces.VideoInfo, proxy string) {
	...
	log.Info("[Downloader]start to download %s, command %s", video.FilePath, arg)
	utils.ExecShell("streamlink", arg...)
}

func DownloadVideo(video *interfaces.VideoInfo, proxy string) string {
	log.Info("[Downloader]%s|%s start to download", video.Provider, video.UsersConfig.Name)
	...
	if !utils.IsFileExist(video.FilePath) {
		log.Info("[Downloader] %s the video file don't exist", video.Title)
		return ""
	}
	log.Info("[Downloader]%s download successfully", video.FilePath)
